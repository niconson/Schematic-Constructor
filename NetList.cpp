...
cnet * CNetList::GetFirstNet()
{
	...
	{
		AfxMessageBox("  Fatal overflow - CNetList::GetFirstNet().cpp( m_pos_i >= MAX_ITERATORS ) Reset m_pos_i at -1"); // fatal overflow
		m_pos_i = -1;
		return NULL;
	}
...
int CNetList::GetNumNets()
{
	int count = 0;
	for( cnet * ni=GetFirstNet(); ni; ni=GetNextNet() )
		count++;
	return count;
}
...
//===============================================================================================
int CNetList::GetPinIndexBy_XY_ForPart( cpart * part, int x, int y )
{
	if( part->shape )
	{
		int RET_I = -1;
		double min_dst = INT_MAX;
		for( int i=0; i<part->shape->GetNumPins(); i++ )
		{
			...
			else if( dst == min_dst && pl < LAY_ADD_1 )
			{
				min_dst = dst;
				RET_I = i;
			}
		}
		if( min_dst < m_pcbu_per_wu )
			return RET_I;
	}
	return -1;
}
...
int CNetList::AddNetConnect( cnet * net, int p1, int p2, int x1, int y1, int x2, int y2 )
{
	...
	int xf = pf.x;
	int yf = pf.y;
	net->connect[net->nconnects].seg[0].layer = LAY_PIN_NAME;
...
void CNetList::RepairBranch( cnet * net, int ic, BOOL bMove )
{
	...
	if( tee )
	{
		...
		if ( FindTeeVertexInNet( net, tee, &ci, &vi ))
		{
			...
			{
				if( bMove || c->seg[c->nsegs-1].layer == LAY_PIN_NAME )
					MoveVertex(net,ic,c->nsegs,xtee,ytee);
				else
				{
					c->vtx[c->nsegs].tee_ID = 0;
					int new_i = AppendSegment(net,ic,xtee,ytee,LAY_PIN_NAME,0,0,0);
...
int CNetList::MirrorNetConnect( cnet * net, int ic, int num_copper_layers, BOOL bDraw )
{
	...
	cconnect * c = &net->connect[ic];
	// pad layer
	if ( c->vtx[0].pad_layer == LAY_ADD_1 )
		c->vtx[0].pad_layer = LAY_ADD_2;
	else if ( c->vtx[0].pad_layer == LAY_ADD_2 )
		c->vtx[0].pad_layer = LAY_ADD_1;
	if ( c->vtx[c->nsegs].pad_layer == LAY_ADD_1 )
		c->vtx[c->nsegs].pad_layer = LAY_ADD_2;
	else if ( c->vtx[c->nsegs].pad_layer == LAY_ADD_2 )
		c->vtx[c->nsegs].pad_layer = LAY_ADD_1;
	//
	int lay;
	for (int is=0; is<c->nsegs; is++)
	{
		...
		if ( lay == LAY_ADD_1 )
			lay = LAY_ADD_2;
		else if ( lay == LAY_ADD_2 )
			lay = LAY_ADD_1;
		else if ( lay > LAY_ADD_1 )
		{
			lay -= LAY_ADD_2;
...
void CNetList::CleanUpConnections( cnet * net, CString * logstr )
{
	for( int ic=net->nconnects-1; ic>=0; ic-- )   
	{
		...
		if( c->end_pin == cconnect::NO_END && c->nsegs == 1 )
		{
			...
			cseg * end_s = &c->seg[0];
			if( end_v->tee_ID == 0 && end_v->via_w == 0 && end_s->layer == LAY_PIN_NAME )
			{
				if( logstr )
				{
					CString str;
		...
		for( int is=c->nsegs-1; is>0; is-- )
		{
			if( c->seg[is].layer != c->seg[is-1].layer &&
				c->seg[is].layer != LAY_PIN_NAME &&
				c->seg[is-1].layer != LAY_PIN_NAME &&
...
int CNetList::UnrouteNetConnect( cnet * net, int ic )
{
	...
	for( int is=0; is<c->nsegs; is++ )
	{
		...
		net->connect[ic].seg[is].layer = LAY_PIN_NAME;
	}
...
void CNetList::MergeUnroutedSegments( cnet * net, int ic, BOOL bDraw )
{
	...
	for( int is=c->nsegs-2; is>=0; is-- )
	{
		...
		cseg * s = &c->seg[is];
		if( post_s->layer == LAY_PIN_NAME && s->layer == LAY_PIN_NAME
...
int CNetList::RemoveSegment( cnet * net, int ic, int is )
{
	...
	if( net->connect[ic].end_pin == cconnect::NO_END )
	{
		...
		if( c->nsegs && c->end_pin ==cconnect::NO_END )
		{
			if( c->seg[c->nsegs-1].layer == LAY_PIN_NAME 
...
{
	if( layer == LAY_FOOTPRINT || layer == LAY_PART_VALUE )
		layer += 2;
	cconnect * c = &net->connect[ic];
	...
	if( iseg == 0 )
	{
		...
		if( pad_layer != LAY_NET_NAME && layer != pad_layer )
			return 1;
	}
	...
	if( dir == 0 )
	{
		...
		{ 
			...
			else if( iseg == (c->nsegs-1) )
			{
				...
				int pad_layer = c->vtx[iseg+1].pad_layer;
				if( pad_layer == LAY_NET_NAME || layer == LAY_PIN_NAME
					|| (pad_layer == LAY_ADD_1 && layer == LAY_ADD_1)
					|| (pad_layer == LAY_ADD_2 && layer == LAY_ADD_2) )
				{
...
//	id id;
	cconnect * c = &net->connect[ic];
	if( c->seg[is].layer != LAY_PIN_NAME && w )
	{
		c->seg[is].width = w;
	}
...
void CNetList::SwapConnects (cnet * n1, cnet * n2, id id1, id id2)
{
	...
	for (int i=0; i<n2->connect[id2.i].nsegs; i++)
	{
		AppendSegment(n1, ic1,	n2->connect[id2.i].vtx[i+1].x, 
								...
								i?n2->connect[id2.i].seg[i].layer:LAY_PIN_NAME, 
	...
	if (end_pin1 >= 0)
	{
		...
		n1->connect[ic1].seg[n1->connect[ic1].nsegs-1].layer = LAY_PIN_NAME;
	...
	for (int i=0; i<n1->connect[id1.i].nsegs; i++)
	{
		AppendSegment(n2, ic2,	n1->connect[id1.i].vtx[i+1].x, 
								...
								i?n1->connect[id1.i].seg[i].layer:LAY_PIN_NAME, 
	...
	if (end_pin2 >= 0)
	{
		...
		n2->connect[ic2].seg[n2->connect[ic2].nsegs-1].layer = LAY_PIN_NAME;
		...
		for( int ic=0; ic<net1->nconnects; ic++ )
		{
			...
			if( nsegs )
			{
				...
				if( p1 == net1_pin_index )
				{
					...
					if( c->seg[0].layer != LAY_PIN_NAME )
					{
						c->seg[0].width = 0;
						c->seg[0].layer = LAY_PIN_NAME;
				...
				if( p2 == net1_pin_index )
				{
					...
					if( c->seg[nsegs-1].layer != LAY_PIN_NAME )
					{
						c->seg[nsegs-1].width = 0;
						c->seg[nsegs-1].layer = LAY_PIN_NAME;
						MergeUnroutedSegments( net1, ic );
						nsegs = c->nsegs;
					}
					// modify vertex position and layer
	...
	if( net1 )
	{
		...
		for( int ic=0; ic<net1->nconnects; ic++ )
		{
			...
			if( nsegs )
			{
				...
				{
					// starting pin is on part, unroute first segment
					if( net1->connect[ic].seg[0].layer != LAY_PIN_NAME )
					{
						net1->connect[ic].seg[0].width = 0;
						net1->connect[ic].seg[0].layer = LAY_PIN_NAME;
						MergeUnroutedSegments( net1, ic );
				...
				{
					// ending pin is on part, unroute last segment
					if( net1->connect[ic].seg[nsegs-1].layer != LAY_PIN_NAME )
					{
						net1->connect[ic].seg[nsegs-1].width = 0;
						net1->connect[ic].seg[nsegs-1].layer = LAY_PIN_NAME;
						MergeUnroutedSegments( net1, ic );
						nsegs = net1->connect[ic].nsegs;
					}
					// modify vertex position and layer
	...
	if( net2 )
	{
		...
		for( int ic=0; ic<net2->nconnects; ic++ )
		{
			...
			if( nsegs )
			{
				...
				{
					// starting pin is on part, unroute first segment
					if( net2->connect[ic].seg[0].layer != LAY_PIN_NAME )
					{
						net2->connect[ic].seg[0].width = 0;
						net2->connect[ic].seg[0].layer = LAY_PIN_NAME;
						MergeUnroutedSegments( net2, ic );
				...
				{
					// ending pin is on part, unroute last segment
					if( net2->connect[ic].seg[nsegs-1].layer != LAY_PIN_NAME )
					{
						net2->connect[ic].seg[nsegs-1].width = 0;
						net2->connect[ic].seg[nsegs-1].layer = LAY_PIN_NAME;
						MergeUnroutedSegments( net2, ic );
						nsegs = net2->connect[ic].nsegs;
					}
					// modify vertex position and layer
				...
				if( nsegs )
				{
					...
					if( net->pin[p1].part == part )
					{
							 ...
							 net->connect[ic].vtx[0].selected == 0)
						{
								...
								net->connect[ic].seg[0].layer = LAY_PIN_NAME;
								MergeUnroutedSegments( net, ic );
						}
						nsegs = c->nsegs;
						...
						{
							// SMT pad on top
							v0->pad_layer = LAY_ADD_1;
						}
						else
						{
							// SMT pad on bottom
							v0->pad_layer = LAY_ADD_2;
						}
					...
					if( p2 != cconnect::NO_END )
					{
						if( net->pin[p2].part == part )
						{
								...
								net->connect[ic].vtx[nsegs].selected == 0) 
							{
									...
									net->connect[ic].seg[nsegs-1].layer = LAY_PIN_NAME;
									MergeUnroutedSegments( net, ic );
							}
							nsegs = c->nsegs;
							// modify vertex position and layer
							...
							{
								// through-hole pad
								c->vtx[nsegs].pad_layer = LAY_NET_NAME;
							}
							...
							{
								// SMT pad, part on top
								c->vtx[nsegs].pad_layer = LAY_ADD_1;
							}
							else
							{
								// SMT pad, part on bottom
								c->vtx[nsegs].pad_layer = LAY_ADD_2;
							}
...
// to this part, and update positions of endpoints
// 
int CNetList::PartFootprintChanged( cpart * part, CString * PINS )
{
	POSITION pos;
	CString name;
	void * ptr;
		...
		for( int ic=net->nconnects-1; ic>=0; ic-- )
		{
			...
			if( nsegs )
			{
				...
				if( net->pin[p1].part == part )
				{
					...
					if( pin_index1 == -1 )
					{
						pin_index1 = GetPinIndexBy_XY_ForPart( part, old_x, old_y );
						if( pin_index1 == -1 || PINS == NULL )
						{
							// no, remove connection
							RemoveNetConnect( net, ic, FALSE );
							continue;
						...
						{
							CString pin_name_mod = part->shape->GetPinNameByIndex( pin_index1 );
							CString pTest;
							pTest.Format("_%s%s_",pin_name1,pin_name_mod);
							if( PINS->Find(pTest) <= 0 )
							{
								*PINS += pTest;
								CString question;
							...
							int ipp = GetNetPinIndex( net, &part->ref_des, &pin_name1 );
							if( ipp == -1 )
								ASSERT(0);
							cnet * p_net = FindPin( &part->ref_des, &pin_name_mod );
							if( p_net == NULL )
								net->pin[ipp].pin_name = pin_name_mod;
							else if( p_net != net )
							{
								// no, remove connection
								CString mess;
								...
								c->start_pin = GetNetPinIndex( net, &part->ref_des, &pin_name_mod );
						}
					}
					...
					int new_layer = LAY_ADD_1;
					if( part->shape->m_padstack[pin_index1].hole_size )
						new_layer = LAY_NET_NAME;
					else if( part->side == 0 )
					{
					...
					if( old_x != new_x || old_y != new_y || !layer_ok )
					{
						...
						if( old_layer != LAY_PIN_NAME && !layer_ok )
						{
							net->connect[ic].seg[0].width = 0;
							net->connect[ic].seg[0].layer = LAY_PIN_NAME;
							MergeUnroutedSegments( net, ic );
				...
				if( net->pin[p2].part == part )
				{
					...
					if( pin_index2 == -1 )
					{
						pin_index2 = GetPinIndexBy_XY_ForPart( part, old_x, old_y );
						if( pin_index2 == -1 || PINS == NULL )
						{
							// no, remove connection
							RemoveNetConnect( net, ic, FALSE );
							continue;
						...
						{
							CString pin_name_mod = part->shape->GetPinNameByIndex( pin_index2 );
							CString pTest;
							pTest.Format("_%s%s_",pin_name2,pin_name_mod);
							if( PINS->Find(pTest) <= 0 )
							{
								*PINS += pTest;
								CString question;
							...
							int ipp = GetNetPinIndex( net, &part->ref_des, &pin_name2 );
							if( ipp == -1 )
								ASSERT(0);
							cnet * p_net = FindPin( &part->ref_des, &pin_name_mod );
							if( p_net == NULL )
							...
							else if( p_net != net )
							{
								...
								c->end_pin = GetNetPinIndex( net, &part->ref_des, &pin_name_mod );
						}
					}
					// rehook pin to net
					part->pin[pin_index2].net = net;
					...
					if( old_x != new_x || old_y != new_y || !layer_ok )
					{
						...
						// yes, unroute if necessary and update connection
						if( c->seg[nsegs-1].layer != LAY_PIN_NAME && !layer_ok )
						{
							net->connect[ic].seg[nsegs-1].width = 0;
							net->connect[ic].seg[nsegs-1].layer = LAY_PIN_NAME;
							MergeUnroutedSegments( net, ic );
	...
	for( int ic=0; ic<net->nconnects; ic++ )
	{
		...
		for( int iv=1; iv<c->nsegs; iv++ )
		{
			...
			if( v->via_w > 0 || layer == 0 || layer == pre_s->layer || layer == post_s->layer
				|| (pre_s->layer == LAY_PIN_NAME && post_s->layer == LAY_PIN_NAME) )
			{
...
int CNetList::CompleteArea( cnet * net, int iarea, int style )
{
	...
	if( al == LAY_FOOTPRINT || al == LAY_PART_VALUE )
...
// if layer == LAY_NET_NAME, matches any layer
// if so, return iarea
//
BOOL CNetList::TestPointInArea( cnet * net, int x, int y, int layer, int * iarea )
{
	...
	for( int ia=0; ia<net->nareas; ia++ )
	{
		...
		///	continue;//  area Ghost
		if( ( a->poly->GetLayer() == layer && !invert ) || ( a->poly->GetLayer() != layer && invert ) || layer == LAY_NET_NAME )
...
void CNetList::HighlightNetVertices( cnet * net, BOOL vias_only, BOOL IncludeEndVias )
{
	for( int ic=0; ic<net->nconnects; ic++ )
	{
		if (net->connect[ic].nsegs == 1 && net->connect[ic].seg[0].layer == LAY_PIN_NAME ){}		
		else 
		{
			cconnect * c = &net->connect[ic];
...
void CNetList::HighlightSegment( cnet * net, int ic, int iseg, int fClearance, int bTRANSPARENT, BOOL W0 )
{
	...
	{
		c->seg[iseg].dl_el->transparent = 0;
		if( bTRANSPARENT || W0 || c->seg[iseg].layer == LAY_PIN_NAME )
		{
			dl_element * sel = m_dlist->Cpy( c->seg[iseg].dl_el );
			sel->layers_bitmap = 0;
			if( bTRANSPARENT )
				sel->transparent = bTRANSPARENT;
			if( c->seg[iseg].layer == LAY_PIN_NAME )
				sel->el_w = 10;
			else if( W0 )
				sel->el_w = 1;
			...
			if( bFound )
			{
				if( net->connect[tee_ic].vtx[tee_iv].via_w == 0 )
					if( net->connect[tee_ic].seg[tee_iv-1].layer != net->connect[ic].seg[ivtx-1].layer )
						if( net->connect[tee_ic].seg[tee_iv-1].layer >= LAY_ADD_1 )
							if( net->connect[ic].seg[ivtx-1].layer >= LAY_ADD_1 )
							{
...
void CNetList::SetNetVisibility( cnet * net, BOOL visible )
{
	...
	else
	{
		...
		for( int ic=0; ic<net->nconnects; ic++ )
		{
			...
			for( int is=0; is<c->nsegs; is++ )
			{
				if( c->seg[is].layer == LAY_PIN_NAME )
				{
...
void CNetList::ReassignCopperLayers( int n_new_layers, int * layer )
{
	...
	while( net )
	{
		for( int ic=0; ic<net->nconnects; ic++ )
		{
			...
			for( int is=0; is<c->nsegs; is++ )
			{
				...
				if( old_layer >= LAY_ADD_1 )
				{
					...
					if( new_layer == -1 )
					{
						...
						s->layer = new_layer + LAY_ADD_1;
				}
			}
			// check for first or last segments connected to SMT pins
			cvertex * v = &c->vtx[0];
			cseg * s = &c->seg[0];
			if( v->pad_layer && v->pad_layer != LAY_NET_NAME && s->layer != v->pad_layer )
			{
				net->connect[ic].seg[0].width = 0;
				net->connect[ic].seg[0].layer = LAY_PIN_NAME;
			}
			if( c->end_pin != cconnect::NO_END )
			{
				...
				if( v->pad_layer && v->pad_layer != LAY_NET_NAME && s->layer != v->pad_layer )
				{
					...
					net->connect[ic].seg[c->nsegs-1].layer = LAY_PIN_NAME;
				}
			}
			MergeUnroutedSegments( net, ic );
		}
		...
		for( int ia=net->nareas-1; ia>=0; ia-- )
		{
			...
			{
				poly->SetLayer( new_layer + LAY_ADD_1 );
				poly->Draw();
			}
		}
...
undo_area * CNetList::CreateAreaUndoRecord( cnet * net, int iarea, int type )
{
	...
	strcpy( un_a->net_name, net->name );
	un_a->merge = net->area[iarea].poly->GetMerge();
	un_a->sub_merge = net->area[iarea].poly->GetMerge(TRUE);
	un_a->iarea = iarea;
	un_a->ncorners = nc;
					...
					|| type == UNDO_AREA_DELETE )
			{
				...
				for( int ic=1; ic<a->ncorners; ic++ )
				{
					...
					if( ic == (a->ncorners-1) )
					{
						...
						net->area[a->iarea].poly->SetMerge( a->merge );
						net->area[a->iarea].poly->SetMerge( a->sub_merge, TRUE );
					}
		...
		for( int iend=0; iend<2; iend++ )
		{
			...
			for( inode=0; inode<nodes->GetSize(); inode++ )
			{
				...
				if( abs(pt->x-node->x)<tolerance && abs(pt->y-node->y)<tolerance 
					&& ( path->layer == node->layer || node->layer == LAY_NET_NAME ) )
				{
					// match, hook it up
				...
				while( !bFailed )
				{
					...
					if( !bFailed )
					{
						...
						for( int iv=1; iv<c->nsegs; iv++ )
						{
							...
							if( pre_seg->layer != post_seg->layer && !TestPointInArea( net, v->x, v->y, LAY_NET_NAME, NULL ) )
