...
void CFootprintView::MirrorGroup()
{
	...
	for( int i=m_fp.m_outline_poly.GetSize()-1; i>=0; i-- )
	{
		...
		if( bLayer && bFlip )
		{
			...
			int inc = 1;
			if( lay == LAY_FP_TOP_COPPER || lay == LAY_FP_BOTTOM_COPPER )
				inc = 2;
			if( Get_fp_Side( lay ) == 0 )
				m_fp.m_outline_poly[i].SetLayer(lay+inc);
			else if( Get_fp_Side( lay ) == 1 )
				m_fp.m_outline_poly[i].SetLayer(lay-inc);
		}
	}
	// pads
...
void CFootprintView::DuplicateGroup()
{
	...
	{
		bFirst = 0;
		AfxMessageBox("Attention! The names of the pins remain the same (created multipins). Then you can manually change the pin names at your discretion.");
	}
	...
	for( int i=num_items-1; i>=0; i-- )
		for( int ii=0; ii<m_fp.m_outline_poly[i].GetNumCorners(); ii++ )
			...
			{
				num_sel_items++;
				m_fp.m_outline_poly[i].SetSel(0,1);
				break;
			}
...
void CFootprintView::HandleKeyPress(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
	...
	case CUR_FP_POLY_SIDE_SELECTED:
		...
		{
			int gl = Get_fp_Side(m_fp.m_outline_poly[m_sel_id.i].GetLayer());
			if( gl == 0 )
				MoveToLayer(LAY_FP_TOP_COPPER);
			else if( gl == 1 )
				MoveToLayer(LAY_FP_BOTTOM_COPPER);
		}
		else if( fk == FK_FP_OP_MAKE_SILK	)
		{
			...
			if( gl == 0 )
				...
				MoveToLayer(LAY_FP_SILK_BOTTOM);
		}
		else if( fk == FK_FP_OP_MAKE_NOTES	)
		{
			int gl = Get_fp_Side(m_fp.m_outline_poly[m_sel_id.i].GetLayer());
			if( gl == 0 )
				MoveToLayer(LAY_PART_NAME);
			else if( gl == 1 )
				MoveToLayer(LAY_PIN_LINE);
		}
		FootprintModified( TRUE );
		break;
	...
	case CUR_FP_TEXT_SELECTED:
		...
		{
			int gl = Get_fp_Side( m_sel_text->m_layer );
			if( gl == 0 )
				MoveToLayer(LAY_FP_TOP_COPPER);
			else if( gl == 1 )
				MoveToLayer(LAY_FP_BOTTOM_COPPER);
...
void CFootprintView::SetFKText( int mode )
{
	...
	case CUR_FP_TEXT_SELECTED:
		...
		m_fkey_option[4] = FK_FP_DELETE_TEXT;
		m_fkey_option[5] = FK_FP_OP_MAKE_COPPER;
		m_fkey_option[6] = FK_FP_OP_MAKE_SILK;
		m_fkey_option[7] = FK_FP_OP_MAKE_NOTES;
		break;
	...
	case CUR_FP_POLY_SIDE_SELECTED: 
		{
			...
			int sst = m_fp.m_outline_poly[m_sel_id.i].GetSideStyle(m_sel_id.ii);			
			int x1 = m_fp.m_outline_poly[m_sel_id.i].GetX(m_sel_id.ii);
			int y1 = m_fp.m_outline_poly[m_sel_id.i].GetY(m_sel_id.ii);	
			int x2 = m_fp.m_outline_poly[m_sel_id.i].GetX(m_fp.m_outline_poly[m_sel_id.i].GetIndexCornerNext(m_sel_id.ii));
			int y2 = m_fp.m_outline_poly[m_sel_id.i].GetY(m_fp.m_outline_poly[m_sel_id.i].GetIndexCornerNext(m_sel_id.ii));
			int len1 = Distance( x1,y1,x2,y2,sst );
			int len2 = m_fp.m_outline_poly[m_sel_id.i].GetLength();
			int len3 = abs(x1-x2);
			int len4 = abs(y1-y2);
			::MakeCStringFromDimension( &str1, len1, m_units,1,1,1,2);
