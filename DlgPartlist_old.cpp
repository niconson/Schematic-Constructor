...
void CDlgPartlist::Initialize( CPartList * plist,
			...
			int units, CDlgLog * log, int lock_netlist, CString * part_search )
{
	m_n_sel = 0;
	m_units = units;
	m_plist = plist;
...
void CDlgPartlist::OnBnClickedButtonEdit()
{
	...
	// adequacy test
	if( m_check_footprint.GetCheck() )
	{
		CString prev_f="";
		if( ::pl[i].shape )
			prev_f = ::pl[i].shape->m_name;
		while( pos && iItem >= 0 )
		{
			iItem = m_list_ctrl.GetNextSelectedItem(pos);
			int ip = m_list_ctrl.GetItemData(iItem);
			if( ::pl[ip].shape )
			{
				if( prev_f.GetLength() )
	...
	dlg.Initialize( &::pl, i, FALSE, FALSE, bMultiple, multiple_mask, m_lock_netlist,
		m_footprint_cache_map, m_footlibfoldermap, m_units, m_dlg_log, m_part_search );
	int ret = dlg.DoModal();
	if( ret == IDOK )
	{
		CString str;
