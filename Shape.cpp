	...
	try
	{
		...
		while( 1 )
		{
			...
			else if( key_str == "text" && np >= 7 )
			{
				...
				m_tl->AddText( x, y, angle, layer, font_size, stroke_w, &p[0] );
			}
...
int CShape::ComparePins( CShape * shape )
{
	// padstacks
	...
	for( int i=0; i<np; i++ )
	{
		if( shape->GetPinIndexByName( m_padstack[i].name, -1 ) == -1 )
			return 0;
		int bad = 1;
		for( int f=shape->GetPinIndexByName( m_padstack[i].name, -1 ); f>=0; f=shape->GetPinIndexByName( m_padstack[i].name, f ) )
		{
			if( abs( m_padstack[i].x_rel-shape->m_padstack[f].x_rel ) < _2540 &&
				abs( m_padstack[i].y_rel-shape->m_padstack[f].y_rel ) < _2540 )
			{
				bad = 0;
				break;
			}
