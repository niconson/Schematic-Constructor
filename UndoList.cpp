...
void CUndoList::DropFirstEvent()
{
	...
	do
	{
		...
		if( first->ptr )
			first->callback( first->ptr, FALSE ); // allow callback to destroy record
		delete first;
		m_num_items--;
		bValidItem = ( m_start.next->ptr || m_start.next->callback );
...
//
void CUndoList::Push( void * ptr, UndoCallbackPtr callback, int size )
{
