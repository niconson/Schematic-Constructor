...
BEGIN_MESSAGE_MAP(CMyToolBar, CToolBar)
	...
	ON_CBN_SELENDOK( IDC_COMBO_TEXT_GRID, OnSelectTextGrid )
			  ...
			  int units )
{
	...
	if(ag == 45 || ag == 90)
		m_ctlComboSnapAngle.SetCurSel( ag/45-1 );
	else
		m_ctlComboSnapAngle.SetCurSel( 2 );
}
...
void CMyToolBar::OnSelectTextGrid()
{
	CString str;

	int cur_sel = m_ctlComboRoutingGrid.GetCurSel();
	AfxGetMainWnd()->SendMessage( WM_USER_TEXT_GRID, WM_BY_INDEX, cur_sel );
	return;
}
